...
import { createDesignation, editDesignation, getDesignation } from '../../../../api';
import { formBtn1, formBtn2, inputClass, labelClass, tableBtn } from '../../../../utils/CustomClass';
import { setDesignation } from '../../../../redux/Slices/masterSlice';
import { useDispatch } from 'react-redux';
import { toast } from 'react-toastify';
import LoadBox from '../../../Loader/LoadBox';
import Error from '../../../Errors/Error';

export default function StorageDesignationForm(props) {
    ...
    const onSubmit = async (data) => {
        if (props?.button !== 'edit') {
            try {
                setLoader(true)
                createDesignation(data).then((res) => {
            ...
            try {
                setLoader(true)
                editDesignation(props?.data?.id, data).then((res) => {
                                        ...
                                        <form onSubmit={handleSubmit(onSubmit)} >
                                            <div className="py-4 mx-4 grid grid-cols-1 gap-x-3 gap-y-3 customBox">
                                                <div className="">
                                                    ...
                                                    <input
                                                        type="text"
                                                        placeholder='Designaton Name'
                                                        className={inputClass}
                                                        {...register('name', { required: true })}
                                                    />
                                                    {errors.name && <Error title='Designaton is required*' />}
                                                </div>
                                            </div>
